    ...
    for (let y = 0; y < height; y += verticalLineSpacing) {
        ...
        for (let x = spacing; x < width; x += spacing) {
            ...
            if (contrast !== 0) {
                ...
                b = (0.2125 * ((contrastFactor * (pixels.data[4 * currentHorizontalPixelIndex]       - 128) + 128) + brightness)) +
                    (0.7154 * ((contrastFactor * (pixels.data[4 * (currentHorizontalPixelIndex + 1)] - 128) + 128) + brightness)) +
                    (0.0721 * ((contrastFactor * (pixels.data[4 * (currentHorizontalPixelIndex + 2)] - 128) + 128) + brightness));
            } else {
                b = (0.2125 * (pixels.data[4 * currentHorizontalPixelIndex]       + brightness)) +
                    (0.7154 * (pixels.data[4 * (currentHorizontalPixelIndex + 1)] + brightness)) +
                    (0.0721 * (pixels.data[4 * (currentHorizontalPixelIndex + 2)] + brightness));
            }
            if (black) {
